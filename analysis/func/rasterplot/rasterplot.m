...
% Apr 4, 2022: edited by Max Levinson
...
%%%%%%%%% Code Begins %%%%%%%%%%%%
switch nin
    case 3 % no color supplied; no handle so plot in a separate figure
        plotcolor = 'k';
          figure;
  hresp=gca;
  fs=defaultfs;
 case 4%3 %color supplied; no handle so plot in a separate figure
  colorlist = varargin{1}; plotcolor = {};
  for cl = 1:numel(colorlist)
      plotcolor{cl} = useclrs{colorlist(cl)};
  end
     figure;
  hresp=gca;
  fs=defaultfs;
 case 5%4 %handle supplied
 colorlist = varargin{1}; plotcolor = {};
  for cl = 1:numel(colorlist)
      plotcolor{cl} = useclrs{colorlist(cl)};
  end
 ...
 % plot spikes
  ...
  axes(hresp);
  h=plot(xx, yy, 'Color', plotcolor, 'linewidth',plotwidth);
